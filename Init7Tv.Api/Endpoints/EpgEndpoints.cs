...
public static class EpgEndpoints
{
    ...
    private static async Task<IResult> GetEpg(string canonicalName, int? day, IEpgService epgService)
    {
        return (await epgService.GetEpg(canonicalName, day ?? 0)).ToHttpResult();
    }
}